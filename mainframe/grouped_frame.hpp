...
struct get_result_columns_from_args<frame<Ts...>, Op<Ind>, Ops...>
{
    ...
    using remaining_columns = 
        typename get_result_columns_from_args<frame<Ts...>, Ops...>::type;
    using type = 
        typename detail::prepend<series<ind_type>, remaining_columns>::type;
};
...
struct get_result_columns_from_args<frame<Ts...>, count_op, Ops...>
{
    using remaining_columns = 
        typename get_result_columns_from_args<frame<Ts...>, Ops...>::type;
    using type = 
        typename detail::prepend<series<size_t>, remaining_columns>::type;
};
...
{
    using ind_type = typename detail::pack_element<Ind, Ts...>::type;
    using remaining_frame_type = 
        typename get_frame_from_args<frame<Ts...>, Ops...>::type;
    using type = 
        typename detail::prepend<ind_type, remaining_frame_type>::type;
};
...
struct get_frame_from_args<frame<Ts...>, count_op, Ops...>
{
    using remaining_frame_type = 
        typename get_frame_from_args<frame<Ts...>, Ops...>::type;
    using type = 
        typename detail::prepend<size_t, remaining_frame_type>::type;
};
...
struct is_colind_at_argind<QColInd, ArgInd, std::tuple<Op, Ops...>>
{
    static const bool value =
        is_colind_at_argind<QColInd, ArgInd-1, std::tuple<Ops...>>::value;
};

template<size_t QColInd, template<size_t>typename Op, size_t ColInd, typename... Ops>
struct is_colind_at_argind<QColInd, 0, std::tuple<Op<ColInd>, Ops...>> : 
    std::false_type 
{};

template<size_t QColInd, template<size_t>typename Op, typename... Ops>
struct is_colind_at_argind<QColInd, 0, std::tuple<Op<QColInd>, Ops...>> : 
    std::true_type 
{};

template<size_t QColInd, typename... Ops>
struct is_colind_at_argind<QColInd, 0, std::tuple<count_op, Ops...>> : 
    std::false_type 
{};
...
{
    using index_frame = 
        typename get_index_frame<index_defn<Inds...>, frame<Ts...>>::type;
    using args_frame =
        typename get_frame_from_args<frame<Ts...>, Ops...>::type;
...
class grouped_frame<index_defn<GroupInds...>, Ts...>
{
    ...
    using get_aggregate_frame = 
        detail::get_aggregate_frame<frame<Ts...>, group_index_defn, Ops...>;
...
private:
    ...
    void
    aggregate_count( const std::vector<size_t>& rowinds, 
                      std::tuple<series<Us>...>& result_columns
                      ) const
    {
    ...
    void
    aggregate_count_arg( const std::vector<size_t>& rowinds, 
                      std::tuple<series<Us>...>& result_columns
                      ) const
    {
    ...
    void
    aggregate_column( const std::vector<size_t>& rowinds, 
                      std::tuple<series<Us>...>& result_columns
                      ) const
    {
    ...
    void
    aggregate_column_arg( const std::vector<size_t>& rowinds, 
                          std::tuple<series<Us>...>& result_columns
                          ) const
    {
        (void)result_columns;
    ...
    void
    aggregate_column_arg_op( detail::sum_op<ColInd>, 
    ...
    template<size_t ColInd, typename T>
    void
    aggregate_column_arg_op( detail::max_op<ColInd>, 
                             const std::vector<size_t>& rowinds, 
                             series<T>& result_column ) const
    {
        using std::max;
    ...
    template<size_t ColInd, typename T>
    void
    aggregate_column_arg_op( detail::min_op<ColInd>, 
                             const std::vector<size_t>& rowinds, 
                             series<T>& result_column ) const
    {
        using std::min;
    ...
    template<size_t Ind, typename... Us>
    void
    vectorize_result_columns( std::tuple<series<Us>...>& result_columns, std::vector<useries>& us ) const
    {
